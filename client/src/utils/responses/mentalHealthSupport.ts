...
export const getMentalHealthSupport = (input: string): string => {
  const lowerInput = input.toLowerCase();
  
  if (lowerInput.includes('depression') || lowerInput.includes('sad') || lowerInput.includes('depressed')) {
    const response = mentalHealthResponses.depression;
    return `${response.message}\n\nHere are some resources that might help:\n- ${response.resources.join('\n- ')}\n\n${response.followUp}`;
  }
  
  if (lowerInput.includes('anxiety') || lowerInput.includes('anxious') || lowerInput.includes('worried')) {
    const response = mentalHealthResponses.anxiety;
    return `${response.message}\n\nHere are some resources that might help:\n- ${response.resources.join('\n- ')}\n\n${response.followUp}`;
  }
  
  if (lowerInput.includes('stress') || lowerInput.includes('overwhelmed') || lowerInput.includes('pressure')) {
    const response = mentalHealthResponses.stress;
    return `${response.message}\n\nHere are some resources that might help:\n- ${response.resources.join('\n- ')}\n\n${response.followUp}`;
  }
  
  return "I'm here to support you with any emotional challenges you might be facing. If you're experiencing stress, anxiety, or depression, please let me know so I can provide more specific guidance. Remember, it's important to reach out to mental health professionals for personalized support.";
};